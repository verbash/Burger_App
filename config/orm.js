...
var orm = {
    ...
    insertOne: function(newBurger, callback) {
        var s = "INSERT INTO " + tableName + "(burger_name, devoured) VALUES(" + newBurger + ", false);"
        connection.query(s, function(err, result) {
            if (err) {
                throw err;
            }
    ...
    updateOne: function(eatBurger, callback) {
        var s = "UPDATE" + tableName + "SET devoured = true WHERE burger_name = " + eatBurger + ";";
        connection.query(s, function(err,result) {
            if (err) {
                throw err;
            }
